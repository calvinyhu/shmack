...
getFiles = (dir, filter, found) => {
  ...
  files.forEach(file => {
    ...
    if (stat.isDirectory()) getFiles(filename, filter, found);
    else if (filename.includes(filter)) found.push(filename);
  });
};
...
gulp.task(WATCH_SCSS, () => {
  gulp.watch('./src/*.scss', [COMPILE_SCSS_FORCE]);
